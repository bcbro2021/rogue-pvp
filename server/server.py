...
class TcpServer(Thread):
    def __init__(self, tcp_port, rooms, lock):
        ...
        """
        Thread.__init__(self)
        self.map = generator.create_island_map(MAP_WIDTH,MAP_HEIGHT,MAP_WIDTH/2)
        self.text_map = generator.create_text_map(self.map)
        self.bushes = generator.generate_deco(self.map,rarity=70)
    ...
    def route(self,
        ...
        if action == "register":
            ...
            # Send a welcome message along with the identifier
            welcome_message = "Welcome to the server, Player {}!".format(client.identifier)
            
            # Add the welcome_message to the response
            response = {
                "success": "True",
        ...
        if action == "getmap": # generates the map for the server
            sock.send(self.text_map.encode())
            return 0

        if action == "getbushes":
            sock.send(pickle.dumps(self.bushes))
            return 0
