...
class Client:
    ...
    def leave_room(self):
        """
        Leave the current room and notify other clients in the same room
        """
        message = json.dumps({
            "action": "leave",
        ...
        # Notify other clients in the same room about the player leaving
        leave_message = json.dumps({
            "action": "send",
            "payload": {"message": f"{self.identifier} has left the room."},
            "room_id": self.room_id,
            "identifier": self.identifier
        })
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.sendto(leave_message.encode(), self.server_udp)
    ...
    def register(self):
        """
        Register the client to the server and get a unique identifier,
        ...
        # Extract and print the welcome message from the server
        if "welcome_message" in response:
            print(f"Welcome to the server! Message from the server: {response['welcome_message']}")

        print(response)
        self.identifier = response['message']
